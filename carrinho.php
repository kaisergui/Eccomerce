...
// Consulta SQL para obter os itens do carrinho de compras do usuário
$sql = "SELECT ic.id_item, p.nome_produto, p.descricao_produto, ic.quantidade_produto, ic.preco_unitario_produto
        FROM itens_carrinho ic
        JOIN produtos p ON ic.id_produto = p.id_produto
        WHERE ic.id_carrinho = (SELECT id_carrinho FROM carrinho_compras WHERE id_usuario = $userId)";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $price = $row['preco_unitario_produto'];

        echo "<div class='cart-item'>";
        echo "<div class='item-details'>";
        echo "<h3 class='item-title'>$productName</h3>";
        echo "<p class='item-description'>$productDescription</p>";
        echo "<p class='item-quantity'>Quantidade: $quantity</p>";
        echo "<p class='item-price'>Preço unitário: R$ $price</p>";
        echo "</div>";
        echo "</div>";
    }

    // Calcular o total do carrinho
    $totalQuery = "SELECT SUM(quantidade_produto * preco_unitario_produto) AS total FROM itens_carrinho WHERE id_carrinho = (SELECT id_carrinho FROM carrinho_compras WHERE id_usuario = $userId)";
    $totalResult = mysqli_query($conn, $totalQuery);
    $totalRow = mysqli_fetch_assoc($totalResult);
    $total = $totalRow['total'];

    echo "<div class='cart-total'>Total: R$ $total</div>";
    echo "<a href='finalizar_pedido.php'><button class='checkout-button'>Finalizar Compra</button></a>";
    echo "</div>";
} else {
    echo "<div class='container6'>";
    echo "<h1>Nenhum item encontrado no carrinho.</h1>";
    echo "</div>";
}
